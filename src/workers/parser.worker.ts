...
export async function parseWorkbookBuffer(buffer: ArrayBuffer): Promise<WorkerParseResult> {
  ...
  return { fileKind, vcenterName, exportTs, sheets, warnings, errors };
}
...
self.onmessage = async (e: MessageEvent) => {
  try {
    ...
    const parsed = await parseWorkbookBuffer(payload.buffer);
    self.postMessage({
      type: "PARSE_COMPLETE",
      payload: parsed,
    });
