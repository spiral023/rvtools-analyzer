...
import type { NormalizedVm, NormalizedHealth, NormalizedSnapshot } from "@/domain/models/types";
...
export default function DailyOps() {
  ...
  const { data: healthEvents = [] } = useQuery({ queryKey: ["health", activeSnapshotIds], queryFn: () => getBySnapshotIds<NormalizedHealth>("entities_health", activeSnapshotIds), enabled: activeSnapshotIds.length > 0 });
  const { data: vmSnapshots = [] } = useQuery({ queryKey: ["vmSnapshots", activeSnapshotIds], queryFn: () => getBySnapshotIds<NormalizedSnapshot>("entities_snapshot", activeSnapshotIds), enabled: activeSnapshotIds.length > 0 });
