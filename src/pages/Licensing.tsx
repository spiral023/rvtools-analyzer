      ...
      {utilizationChart.length > 0 && (
        <div className="rounded-lg border border-border/50 bg-card/30 p-4">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={utilizationChart} layout="vertical"><XAxis type="number" domain={[0, 100]} tick={CHART_AXIS_STYLE} axisLine={false} tickLine={false} /><YAxis type="category" dataKey="name" width={180} tick={{ ...CHART_AXIS_STYLE, fontSize: 10 }} axisLine={false} tickLine={false} /><Tooltip contentStyle={CHART_TOOLTIP_STYLE} /><Bar dataKey="usedPct" radius={[0, 4, 4, 0]}>{utilizationChart.map((e, i) => <Cell key={i} fill={e.usedPct > 95 ? CHART_COLORS.danger : e.usedPct > 85 ? CHART_COLORS.warning : CHART_COLORS.success} />)}</Bar></BarChart>
          </ResponsiveContainer>
        </div>
      )}
