...
import { db, deleteSnapshot, deleteAllData } from "@/data/db";
...
export default function UploadSnapshots() {
  ...
  const { data: snapshots = [], refetch } = useQuery({
    queryKey: ["snapshots"],
    queryFn: () => db.snapshots.orderBy("importedAt").reverse().toArray(),
  });
  ...
  const handleFiles = useCallback(
    async (files: FileList | File[]) => {
      ...
      if (xlsxFiles.length === 0) {
        toast.error("Keine gültige XLSX-Datei ausgewählt.");
        return;
      }
      ...
      for (const file of xlsxFiles) {
        try {
          ...
          if (result.success) {
            toast.success(`"${file.name}" erfolgreich importiert.`);
          } else {
            toast.error(`Fehler bei "${file.name}": ${result.errors.join(", ")}`);
          }
        } catch (err) {
    ...
    <div className="space-y-6 animate-fade-in">
        ...
        <Dialog open={deleteAllOpen} onOpenChange={setDeleteAllOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Dies löscht alle importierten Snapshots, Analysedaten und gespeicherten Einstellungen unwiderruflich aus Ihrem Browser.
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
      ...
      >
        ...
        <p className="mt-3 text-sm font-medium">
          {importing
            ? "Import läuft..."
            : "RVTools XLSX-Datei hierher ziehen oder klicken"}
        </p>
        <p className="mt-1 text-xs text-muted-foreground">
          Mehrere Dateien und wiederholte Uploads pro vCenter möglich
        </p>
      </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="font-semibold text-sm">
                {lastResult.success ? "Import erfolgreich" : "Import fehlgeschlagen"}
              </span>
            </div>
            {lastResult.sheetStats && (
              <p className="text-xs text-muted-foreground mb-2">
                {Object.keys(lastResult.sheetStats).length} Sheets erkannt,{" "}
                {Object.values(lastResult.sheetStats).reduce((s, v) => s + v.rowCount, 0).toLocaleString("de-DE")} Zeilen
              </p>
            )}
              ...
              <div className="mt-2 space-y-1">
                ...
                {lastResult.warnings.length > 10 && (
                  <p className="text-xs text-muted-foreground">
                    ...und {lastResult.warnings.length - 10} weitere Warnungen
                  </p>
                )}
              </div>
            )}
                ...
                <CardContent className="flex items-center justify-between p-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        vCenter: {s.vcenterDisplayName} · Export:{" "}
                        {new Date(s.exportTs).toLocaleString("de-DE")} · Import:{" "}
                        {new Date(s.importedAt).toLocaleString("de-DE")}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {Object.keys(s.sheetStats).length} Sheets,{" "}
                        {Object.values(s.sheetStats)
                          .reduce((sum, v) => sum + v.rowCount, 0)
                  ...
                  <Button
                    ...
                    onClick={() => handleDeleteSnapshot(s.snapshotId)}
                  >
