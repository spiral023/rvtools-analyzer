...
export function NetworkSecurityPanel() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div><InfoTooltip entry={NET_SECURITY_SECTIONS.policiesTable} side="bottom"><h3 className="mb-3 w-fit cursor-help text-sm font-semibold text-muted-foreground">Security Policies ({policies.length})</h3></InfoTooltip><VirtualTable tableId="network/security-policies" columnPicker data={policies} columns={policyColumns} globalFilter={filters.search} /></div>

      {uplinkData.length > 0 && (<div><InfoTooltip entry={NET_SECURITY_SECTIONS.uplinkTable} side="bottom"><h3 className="mb-3 w-fit cursor-help text-sm font-semibold text-muted-foreground">Uplink Redundanz Risiken ({uplinkData.length})</h3></InfoTooltip><VirtualTable tableId="network/uplink-redundancy" columnPicker data={uplinkData} columns={uplinkColumns} globalFilter={filters.search} height={300} /></div>)}
      {teamingData.length > 0 && (<div><InfoTooltip entry={NET_SECURITY_SECTIONS.teamingTable} side="bottom"><h3 className="mb-3 w-fit cursor-help text-sm font-semibold text-muted-foreground">NIC Teaming Auffälligkeiten ({teamingData.length})</h3></InfoTooltip><VirtualTable tableId="network/nic-teaming" columnPicker data={teamingData} columns={teamingColumns} globalFilter={filters.search} height={300} /></div>)}
