...
import type { NormalizedDatastore, NormalizedCluster, NormalizedVm } from "@/domain/models/types";
...
export default function Capacity() {
  ...
  const { data: datastores = [] } = useQuery({ queryKey: ["datastores", activeSnapshotIds], queryFn: () => getBySnapshotIds<NormalizedDatastore>("entities_datastore", activeSnapshotIds), enabled: activeSnapshotIds.length > 0 });
  const { data: clusters = [] } = useQuery({ queryKey: ["clusters", activeSnapshotIds], queryFn: () => getBySnapshotIds<NormalizedCluster>("entities_cluster", activeSnapshotIds), enabled: activeSnapshotIds.length > 0 });
  ...
  const dsChart = useMemo(() => {
    return datastores.filter((d) => d.freePct !== null).map((d) => ({ name: d.name, freePct: Math.round(d.freePct! * 10) / 10 })).sort((a, b) => a.freePct - b.freePct).slice(0, 15);
  }, [datastores]);
          ...
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={dsChart} layout="vertical">
              ...
              <Bar dataKey="freePct" radius={[0, 4, 4, 0]}>
                {dsChart.map((entry, i) => <Cell key={i} fill={entry.freePct < 10 ? "hsl(0, 72%, 51%)" : entry.freePct < 20 ? "hsl(38, 92%, 50%)" : "hsl(152, 69%, 40%)"} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
          ...
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={clusterOvercommit}>
              ...
              <Bar dataKey="ratio" fill="hsl(190, 85%, 48%)" radius={[4, 4, 0, 0]}>
                {clusterOvercommit.map((entry, i) => <Cell key={i} fill={entry.ratio > 5 ? "hsl(0, 72%, 51%)" : entry.ratio > 3 ? "hsl(38, 92%, 50%)" : "hsl(190, 85%, 48%)"} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
