...
export function createFillUpAnalysisRun(input: { name: string; importMeta: VropsTimeSeriesImport; results: readonly FillUpPlanningClusterResult[]; profiles: readonly FillUpWorkloadProfile[]; workloadMix: FillUpWorkloadMix | null; includeN2: boolean; now?: string }): FillUpAnalysisRun {
...
export function buildFillUpRunMarkdown(run: FillUpAnalysisRun): string { return [`# ${run.name}`, "", `Erstellt: ${new Date(run.createdAt).toLocaleString("de-DE")}`, `Import: ${run.importId}`, "", "| Cluster | Mix +VM | N-1 | N-2 | Site | Limiter |", "|---|---:|---|---|---|---|", ...run.results.map((row) => `| ${row.clusterName} | ${row.mixAdditionalVms ?? "—"} | ${row.n1Status} | ${row.n2Status ?? "—"} | ${row.siteFailoverStatus} | ${row.limitingMetric ?? "—"} |`)].join("\n"); }
