...
export function SystemDetailContent({
  ...
  return (
    ...
    >
      <DialogHeader className="relative shrink-0 border-b border-border/70 bg-card px-5 py-4 pr-14 text-left sm:px-7">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="flex min-w-0 items-center gap-3.5">
            <div className="grid size-12 shrink-0 place-items-center rounded-xl bg-primary/10 text-primary shadow-[inset_0_0_0_1px_hsl(var(--primary)/0.12)]">
              {icon}
            </div>
            <div className="min-w-0">
              <p className="mb-1 text-[10px] font-bold uppercase tracking-[0.16em] text-primary">{eyebrow}</p>
              <DialogTitle className="truncate text-xl font-semibold tracking-tight text-balance">
                {title}
              </DialogTitle>
              <DialogDescription className="mt-1 truncate text-xs text-pretty">{subtitle || "Systemdetails"}</DialogDescription>
              {badges && <div className="mt-2 flex flex-wrap gap-1.5">{badges}</div>}
            </div>
          </div>
          <div className="mr-7 flex shrink-0 items-center gap-2 md:mr-0">
            <div className="hidden text-right text-[10px] leading-4 text-muted-foreground lg:block">
              <span className="block font-medium text-foreground">Lokal erzeugt</span>
      ...
      <div className="system-detail-scroll min-h-0 flex-1 overflow-y-auto overscroll-contain">
        <main className="mx-auto w-full max-w-[1380px] space-y-5 p-4 sm:p-6 lg:p-7">{children}</main>
      </div>
    </DialogContent>
  );
}
...
export function DetailNarrative({ children, source }: { children: ReactNode; source?: string }) {
  return (
    <div className="detail-surface relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary/[0.08] via-card to-card p-5 shadow-[var(--detail-surface-shadow)]">
      <div className="absolute inset-y-0 left-0 w-1 bg-primary" />
      <p className="max-w-5xl text-sm leading-6 text-pretty">{children}</p>
      {source && (
        <p className="mt-3 flex items-center gap-1.5 text-[10px] font-medium uppercase tracking-[0.11em] text-muted-foreground">
...
export function DetailKpiGrid({ items }: { items: DetailKpi[] }) {
  return (
    <div className="grid gap-2.5 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6">
      {items.map((item) => (
        <div key={item.label} className="detail-surface min-h-[92px] rounded-xl bg-card p-3.5 shadow-[var(--detail-surface-shadow)]">
          <p className="text-[10px] font-semibold uppercase tracking-[0.11em] text-muted-foreground">{item.label}</p>
          <p className={cn(
            "mt-2 truncate font-mono text-lg font-semibold tabular-nums tracking-tight",
            item.tone === "good" && "text-success",
            item.tone === "warning" && "text-warning",
            item.tone === "critical" && "text-destructive",
          )} title={item.value}>{item.value}</p>
          {item.hint && <p className="mt-1 truncate text-[11px] text-muted-foreground" title={item.hint}>{item.hint}</p>}
        </div>
      ))}
    </div>
  );
