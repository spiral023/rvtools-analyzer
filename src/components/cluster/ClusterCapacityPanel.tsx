...
export function ClusterCapacityPanel({ capacityRows, overcommitRows, hostDensity, clusterDensity, search, onOpenCluster }: ClusterCapacityPanelProps) {
  const riskChart = capacityRows.slice(0, 12).map((row) => ({ ...row, name: `${row.vcenterDisplayName} · ${row.cluster}` }));
        ...
        <section className="rounded-lg border border-border/50 bg-card/30 p-4">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <ScatterChart>
              ...
              <Scatter data={hostDensity}>{hostDensity.map((row) => <Cell key={row.hostKey} fill={hostDensityColor(row.vcpuPerCore)} />)}</Scatter>
            </ScatterChart>
          </ResponsiveContainer>
        </section>
        <section className="rounded-lg border border-border/50 bg-card/30 p-4">
