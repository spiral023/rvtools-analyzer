...
import { MAX_SYSV_BATCH_CONTAINER_BYTES, SYSV_BATCH_SIZE_WARNING_BYTES, type SysvBatchExportRequest } from "@/domain/services/sysvBatchExportService";
          ...
          {exportMode === "batch" && <Card>
            ...
            <CardContent className="space-y-4">
              ...
              {!batchPreviewLoading && !batchPreviewError && batchPreview && <>
                ...
                <div className={cn("rounded-lg border p-4", batchPreview.estimatedCompressedBytes >= SYSV_BATCH_SIZE_WARNING_BYTES ? "border-amber-500/45 bg-amber-500/[0.08]" : "border-primary/25 bg-primary/[0.06]")}>
                  <div className="flex flex-wrap items-center justify-between gap-2"><p className="flex items-center gap-2 font-medium"><Database className="size-4 text-primary" /> Geschätzte Containergröße</p><span className="font-mono text-sm tabular-nums">ca. {formatBytes(batchPreview.estimatedCompressedBytes)} ZIP</span></div>
                  <p className="mt-1 text-xs text-muted-foreground">{formatBytes(batchPreview.estimatedUncompressedBytes)} unkomprimiert · {batchPreview.rootLabel}</p>
                  {batchPreview.estimatedCompressedBytes >= SYSV_BATCH_SIZE_WARNING_BYTES && <p className="mt-2 flex items-start gap-2 text-sm text-amber-800 dark:text-amber-200"><AlertTriangle className="mt-0.5 size-4 shrink-0" />Der Batch ist größer als 500 MB. Der Export benötigt entsprechend Speicher im Browser.</p>}
                  {batchPreview.estimatedCompressedBytes >= MAX_SYSV_BATCH_CONTAINER_BYTES && <p className="mt-2 flex items-start gap-2 text-sm text-destructive"><CircleAlert className="mt-0.5 size-4 shrink-0" />Die Schätzung überschreitet das harte Containerlimit von 3 GB.</p>}
                </div>
                <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border p-3">
                ...
                {batchPreview.targets.length > 0 && <div className="rounded-lg border border-border/70"><div className="border-b border-border/60 px-3 py-2 text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">Zielscopes</div><div className="max-h-64 overflow-y-auto">{batchPreview.targets.slice(0, 24).map(({ target, preview: targetPreview }) => <div key={`${target.path}-${targetPreview.packageId}`} className="flex items-center justify-between gap-3 border-b border-border/40 px-3 py-2 text-sm last:border-0"><span className="min-w-0 truncate" title={target.path}>{target.path}</span><span className="shrink-0 font-mono text-xs tabular-nums text-muted-foreground">{targetPreview.vms.length.toLocaleString("de-DE")} VMs</span></div>)}{batchPreview.targets.length > 24 && <p className="px-3 py-2 text-xs text-muted-foreground">… und {batchPreview.targets.length - 24} weitere Blattpakete</p>}</div></div>}
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 border-t border-border/60 pt-4"><p className="text-xs text-muted-foreground">Der Container enthält <span className="font-medium text-foreground">uebersicht.json</span>, <span className="font-medium text-foreground">uebersicht.csv</span> und einzeln importierbare Blattpakete.</p><Button type="button" onClick={() => setBatchConfirmOpen(true)} disabled={batchPreview.targets.length === 0 || batchPreview.estimatedCompressedBytes >= MAX_SYSV_BATCH_CONTAINER_BYTES || batchExporting} className="gap-2">{batchExporting ? <Loader2 className="size-4 animate-spin" /> : <Download className="size-4" />}Batch-Container erzeugen</Button></div>
              </>}
            </CardContent>
          </Card>}
